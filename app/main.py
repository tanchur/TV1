...
class MortgageApp:
    ...
    def create_widgets(self):
        ...
        # Поля ввода
        ttk.Label(main_frame, text="Сумма кредита (руб):").grid(row=0, column=0, sticky=tk.W, pady=5)
        self.principal_entry = ttk.Entry(main_frame)
        self.principal_entry.grid(row=0, column=1, sticky=(tk.W, tk.E), pady=5)

        ttk.Label(main_frame, text="Процентная ставка (% год):").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.rate_entry = ttk.Entry(main_frame)
        self.rate_entry.grid(row=1, column=1, sticky=(tk.W, tk.E), pady=5)

        ttk.Label(main_frame, text="Срок кредита (лет):").grid(row=2, column=0, sticky=tk.W, pady=5)
        self.years_entry = ttk.Entry(main_frame)
        self.years_entry.grid(row=2, column=1, sticky=(tk.W, tk.E), pady=5)

        # Кнопка расчета
        self.calculate_btn = ttk.Button(main_frame, text="Рассчитать", command=self.calculate)
    ...
    def calculate(self):
        try:
            ...
            results += f"{'Месяц':<6} {'Платеж':<12} {'Основной':<12} {'Проценты':<12} {'Остаток':<12}\n"
            results += "-" * 60 + "\n"

            schedule = calculator.generate_payment_schedule()
            for payment in schedule[:12]:
                results += f"{payment['month']:<6} {payment['payment']:<12,.2f} {payment['principal']:<12,.2f} {payment['interest']:<12,.2f} {payment['balance']:<12,.2f}\n"
